...
plot_violin <- function(input, title, color_by, gene, facet_by = "NA", ncol = "NA", size = 1, colors = "NA"){
  ...
  g <- g + geom_jitter(aes_string(x=color_by, y=gene, col = color_by), width = 0.2, size = size, alpha = 0.5)
  g <- g + geom_violin(aes_string(x=color_by, y=gene, col = color_by), trim = T)
