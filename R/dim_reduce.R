#' Initial Dimension Reduction and Clustering
#'
...
#' @examples
#' filtered_data <- initial_dim_reduce(input = filtered_data, threshold = 5, minCells = 50, nComp_ICA = 10, tSNE_perp = 30)

dim_reduce <- function(input, threshold, minCells, nComp_ICA, tSNE_perp, print_progress=TRUE){
  gCount <- apply(input,1,function(x) length(which(x>=threshold)))
  gene_subset <- rownames(input[(which(gCount >= minCells)),])
  ic <- fastICA(t(log2(input[gene_subset,]+2)-1), n.comp=(nComp_ICA), alg.typ = 'parallel', fun='logcosh', alpha = 1.0, method = 'C', verbose = print_progress)
  colnames(ic$A) <- gene_subset
  rownames(ic$S) <- colnames(input)
  set.seed(100)
  tSNE_result <- Rtsne(ic$S, dims = 2, perplexity = tSNE_perp, theta = 0.5, check_duplicates = F, pca = F, max_iter = 1000, verbose = print_progress)
  row.names(tSNE_result$Y) <- rownames(ic$S)
  colnames(ic$S) <- paste0("ICA Component ", seq(1:ncol(ic$S)))
  colnames(tSNE_result$Y) <- c("x", "y")
  dim_reduce <- cbind(tSNE_result$Y, ic$S)
  return(dim_reduce)
}
