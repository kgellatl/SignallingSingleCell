...
#' @examples
#' plot_violin2(ex_sc, gene = "CXCL13", color_by = "Skin", facet_by = c("Disease", "CellType"), log_scale = T)
#' plot_violin2(ex_sc, gene = "CXCL13", color_by = "Skin", facet_by = c("CellType", "Disease"), spread = T, log_scale = T)
#' @export
#'
plot_violin2 <- function (input, title = "", gene, color_by, log_scale = F,
          colors = NA, facet_by = NA, spread = NA, jitter_pts = T,
          ...
          text_sizes = c(20, 10, 5, 10, 5, 5), alpha = 0.5, theme = "classic")
{
  ...
  if (number_labels == T) {
    g <- g + stat_summary(aes_string(x = color_by, y = "raw"), fun.data = function(x) {return(c(y = -max(df$plot)/30, label = length(x)))}, colour = "black",
                          geom = "text", size = 2)
  ...
  if (plot_mean == TRUE) {
    ...
    g <- g + stat_summary(aes_string(x = color_by, y = "raw"), fun.y = function(x) mean(x)*(scale * 0.5), colour = "black", geom = "point", size = 2)
    g <- g + scale_y_continuous(sec.axis = sec_axis(~./(scale * 0.5), name = "Mean Expression"))
  }
  if (any(!is.na(facet_by))) {
