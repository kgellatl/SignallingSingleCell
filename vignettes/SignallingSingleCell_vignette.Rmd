...
## Constructing the Expression Set Class  

The Expression Set class (ex_sc) is an extremely convienient data structure that contains 3 dataframes. These dataframes contain expression data, cell information, and gene information respectivelty. 
...
ex_sc_example <- construct_ex_sc(sc_dat) # sc_dat == Input expression matrix. This is the only function that does not take the Expression Set Class as an input
...
Now that we have dimension reduced data we can try clustering it! From the UMI plot it looks as though there are 6 clusters.
...
plot_density(ex_sc_example, title = "UMI per cluster", val = "UMI_sum", color_by = "Cluster", statistic = "mean")
```
...
plot_tsne_metadata(ex_sc_example, color_by = "Timepoint", title = "Clusters over Time", facet_by = "Cluster")
...
Forcluster id a function is provided that will id marker genes. This method is much quicker DE and returns genes that are uniquely expressed in each cluster.

```{r, error=FALSE, warning=FALSE, cache=FALSE, include=TRUE}
ex_sc_example <- id_markers(ex_sc_example, print_progress = TRUE) # This is a quick method to find good markers genes for cell identification. These genes get written to fData()
head(fData(ex_sc_example))

marker_list <- return_markers(ex_sc_example, num_markers = 50) # However returning the genes as a list can be helpful

plot_scatter(input = ex_sc_example, title = "Top 2 marker genes for Cluster 3", gene1 = marker_list$Cluster3[1], gene2 = marker_list$Cluster3[2], facet_by = "Cluster", color_by = "Cluster") # A scater plotting function. Note the correlation of the marker genes.

marker_facet <- c("Csf1r", "Lcn2", "Ccr7")

plot_tsne_gene(ex_sc_example, gene = marker_facet, title = "Cluster Markers", density = TRUE)
```

## Data Subsetting

```{r, error=FALSE, warning=FALSE, cache=FALSE, include=TRUE}
ex_sc_example_subset <- subset_ex_sc(ex_sc_example, variable = "Cluster", select = c("Cluster1")) #subsetting can be very handy! If needed chain together several rounds of subsetting.

plot_tsne_gene(ex_sc_example_subset, gene = "Ccl22", facet_by = "Timepoint", title = "Cluster 4 Tnf over Time", density = TRUE) # this function is flexible for plotting any data onto a 2d tsne mapping

plot_violin(ex_sc_example_subset, title = "Cluster 4 Ccl22 over Time", gene = "Ccl22", facet_by = "Cluster", color_by = "Timepoint")
