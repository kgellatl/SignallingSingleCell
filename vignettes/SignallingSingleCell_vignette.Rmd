...
plot_density_ridge(ex_sc_example, title = "UMIs per cluster", val = "Lyz1", color_by = "Cluster")
```
...
```{r, include=TRUE, cache=FALSE, warning=FALSE, error=FALSE}
gene_subset_exp <- subset_genes(ex_sc_example, method = "Expression", threshold = 3, minCells = .01*(ncol(exprs(ex_sc_example)))) # filter genes on expression

gene_subset_var <- subset_genes(ex_sc_example, method = "PCA", threshold = 3, minCells = .01*(ncol(exprs(ex_sc_example))), nComp = 10, cutoff = 0.85) # filter genes on variability
...
plot_density(ex_sc_example_norm, title = "UMIs per cluster", val = "UMI_sum", color_by = "Cluster", statistic = "mean")
```
...
marker_list <- return_markers(ex_sc_example_norm, num_markers = 50) # However returning the genes as a list can be helpful

plot_scatter(input = ex_sc_example_norm, title = "Top 2 markers for Cluster 2", gene1 = marker_list$Cluster2[1], gene2 = marker_list$Cluster2[2], facet_by = "Cluster", color_by = "Cluster") # A scater plotting function. Note the correlation of the marker genes.
...
```{r, error=FALSE, warning=FALSE, cache=FALSE, include=TRUE}
ex_sc_example_subset <- subset_ex_sc(ex_sc_example_norm, variable = "Cluster", select = c("Cluster3")) #subsetting can be very handy! If needed chain together several rounds of subsetting.
