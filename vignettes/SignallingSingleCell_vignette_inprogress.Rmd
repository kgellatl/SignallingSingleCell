...
```{r, include=TRUE, cache=FALSE, warning=FALSE, error=FALSE}
gene_subset1 <- subset_genes(input = exprs(ex_sc_example), threshold = 3, minCells = 30, method = "CV", cutoff = 0.5)
prelim_dims <- dim_reduce(input = exprs(ex_sc_example)[gene_subset,], pre_reduce = "iPCA", nComp = 15, tSNE_perp = 30, print_progress=TRUE)
...
spec <- kknn::specClust(prelim_dims_PCA, centers = 6, method = 'random-walk')
pData(ex_sc_example)$Cluster <- paste0("Cluster", spec$cluster)
