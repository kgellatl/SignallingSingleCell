...
```{r, include=TRUE, cache=FALSE, warning=FALSE, error=FALSE}
prelim_dims <- dim_reduce(input = exprs(ex_sc_example), threshold = 5, minCells = 50, pre_reduce = "iPCA", nComp = 10, tSNE_perp = 30, print_progress=TRUE)
```

From this we can map on our UMI counts before normalization to view if there is any bias. We can also look at a gene of interest.
