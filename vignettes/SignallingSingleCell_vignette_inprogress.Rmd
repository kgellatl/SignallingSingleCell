...
ex_sc_example <- pre_filter(ex_sc_example, threshold = 1, minCells = 10, minUMI = 1010, maxUMI = 10000)  
ex_sc_example <- calc_libsize(ex_sc_example)
draw_density(ex_sc_example, val = "UMIs", statistic = "mean")  
...
```{r, include=TRUE, cache=FALSE, warning=FALSE, error=FALSE}
gene_subset <- subset_genes(ex_sc_example, method = "PCA", threshold = 3, minCells = 30, nComp = 10, cutoff = 0.75)  

ex_sc_example <- dim_reduce(ex_sc_example, genelist = gene_subset, pre_reduce = "iPCA", nComp = 15, tSNE_perp = 30, iterations = 500, print_progress=TRUE)  
...
```{r, include=TRUE, cache=FALSE, warning=FALSE, error=FALSE}
ex_sc_example <- cluster_sc(ex_sc_example, dimension = "Comp", method = "spectral", num_clust = 6)  

plot_tsne_metadata(ex_sc_example, color_by = "Cluster", title = "Clusters")
...
```{r, include=TRUE, cache=FALSE, warning=FALSE, error=FALSE}
pData(ex_sc_example)$Timepoint <- NA
...
plot_tsne_metadata(ex_sc_example, color_by = "Timepoint", title = "Clusters over Time", facet_by = "Cluster", size = 0.2)

# From this we can see that some clusters are forming based on timepoint, while others contain multiple timepoints.
...
```{r, include=TRUE, cache=FALSE, warning=FALSE, error=FALSE}
ex_sc_example <- id_markers(ex_sc_example, num_markers = 10, num_reference = 1000)
marker_list <- return_markers(ex_sc_example)

gene <- marker_list[[6]][2]
